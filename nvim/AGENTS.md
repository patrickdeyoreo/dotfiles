...
- Keymaps start from the `default` preset and override several keys, most notably a smart, multi-key scheme that arbitrates between blink's menu, Copilot's ghost-text suggestion, and NES (see the AI Stack section below for the full behavior): `<Tab>` selects-next when the menu is open, else accepts a Copilot suggestion, else jumps forward in a snippet, else triggers completion on a non-whitespace prefix (literal Tab otherwise); `<M-Tab>` accepts a Copilot suggestion or pending NES edit directly, regardless of blink's state; `<C-Tab>` dismisses blink/the suggestion/any pending NES and inserts a literal tab; `<Esc>` = `{ "cancel", "fallback" }` closes blink without leaving insert mode. `get_copilot_suggestion()`/`get_copilot_nes()` at the top of the file are shared helpers for the `pcall(require(...))` + state checks these keymaps repeat. `<CR>` = `{ "accept", "fallback" }` (confirms only when an entry is selected; `selection.preselect = false`), `<C-q>` hide, `<C-Space>`/`<C-@>` show, `<C-u>`/`<C-d>`/`<PageUp>`/`<PageDown>` scroll docs, `<C-g>` toggle docs, `<C-k>` deferred to the LSP signature mapping.
